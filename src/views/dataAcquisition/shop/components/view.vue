  ...
  >
    <el-form ref="dataForm" :inline="true" :rules="rules" :model="temp" label-width="120px" >
      <el-form-item label="店铺名称" prop="shop_name">
        <el-input v-model.trim="temp.shop_name" placeholder="请输入店铺名称" autocomplete="off" :disabled="isCanView" clearable/>
      </el-form-item>
      <el-form-item label="经营范围" prop="manage">
        <el-input v-model.trim="temp.manage" placeholder="请输入经营范围" autocomplete="off" :disabled="isCanView" clearable/>
      </el-form-item>
      <el-form-item label="法人" prop="legal_person">
        <el-input v-model.trim="temp.legal_person" placeholder="请输入法人" autocomplete="off" :disabled="isCanView" clearable/>
      </el-form-item>
      <el-form-item label="联系方式" prop="relation">
        <el-input v-model.trim="temp.relation" placeholder="请输入联系方式" autocomplete="off" :disabled="isCanView" clearable/>
      </el-form-item>
      <el-form-item label="违章次数" prop="break_rules_count" v-if="this.paraData.operatorType != 'create'">
        <el-input v-model.trim="temp.break_rules_count" placeholder="请输入违章次数" autocomplete="off" :disabled="true" clearable/>
      </el-form-item>
      <el-form-item label="" v-if="this.paraData.operatorType != 'create'">
<!--        <el-button type="primary" class="btn_blue02">添加违章</el-button>-->
      ...
      <el-form-item label="排查次数" prop="check_count" v-if="this.paraData.operatorType != 'create'">
        <el-input v-model.trim="temp.check_count" placeholder="请输入排查次数" autocomplete="off" :disabled="true" clearable/>
      </el-form-item>
      <el-form-item label="" v-if="this.paraData.operatorType != 'create'">
<!--        <el-button type="primary" class="btn_blue02">添加排查</el-button>-->
        <el-button type="primary" class="btn_blue02" @click="handleCheck(temp.id)">排查详情</el-button>
      </el-form-item>
    </el-form>
    <el-form ref="dataForm" :rules="rules" :model="temp" label-width="120px" class="mr_20">
      <el-form-item label="营业执照" prop="manage_images">
        <SingleImage
                v-if="!isCanView"
          :tempUrl="temp.manage_images"
          v-on:imgSrc="hasImgSrc"
        ></SingleImage>
        <img :src="temp.manage_images" class="my_img" v-else/>
      </el-form-item>
    ...
    <checkView :showDialog.sync="dialogCheckVisible" :paraData="checkData"></checkView>
  </myDialog>
</template>

<script>
  import map from '@/components/Map/map' // 引入刚才的map.js 注意路径
  import {storeAdd,storeDetail,storeEdit} from '@/api/data'
  ...
  export default {
    ...
    methods: {
      ...
      hasImgSrc(val) {
        this.temp.manage_images = val.url;
      ...
      getView(){
        storeDetail({id:this.paraData.id}).then(res=>{
          const { id, shop_name, manage, legal_person, relation, manage_images,address,log,lat,check_count,break_rules_count} = res.data;
          this.temp = {id, shop_name, manage, legal_person, relation, manage_images,address,log,lat,check_count,break_rules_count,}
        });
      },
